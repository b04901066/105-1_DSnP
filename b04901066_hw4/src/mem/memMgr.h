...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      t = toSizeT(t);                         // 1.
      if( getRemainSize() < t ) return false; // 4.
      ret = (T*)_ptr;                         // 3.
      _ptr += t;                              // 2.
      return true;
   }
...
class MemRecycleList
{
   ...
   T* popFront() {
      // TODO
      T* pop_temp = _first;
      if(_first != NULL){_first = (T*)*((size_t*)_first);}
      return pop_temp;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
      while((_activeBlock->getNextBlock()) != NULL){ // 1.
         MemBlock<T>* del_temp = _activeBlock;
         _activeBlock = _activeBlock->getNextBlock();
         delete del_temp;
      }
      _activeBlock->reset();                         // 1.4.
      for(size_t i = 0 ; i < R_SIZE ; ++i){          // 2.
         _recycleList[i].reset();
      }
      if( b != 0 && b != _blockSize ){               // 3.
         delete _activeBlock;
         b = toSizeT(b);
         _activeBlock = new MemBlock<T>(0, b);       // 4.
         _blockSize = b;
      }
   }
   // Called by new
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(iter->getArrSize() != n){
         if(iter->getNextList() == NULL){
            iter->setNextList(new MemRecycleList<T>(n));
         }
   ...
   T* getMem(size_t t) {
      ...
      #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << n << "]..." << endl;
         if(ret != NULL)cout << "Succeed" << endl;
      #endif // MEM_DEBUG
      ...
      #ifdef MEM_DEBUG
         cout << "getMem ("<< t <<") from _activeBlock " << _activeBlock << " Memory addr... " << ret << endl;
      #endif // MEM_DEBUG
         return ret;
      }
	 ...
	 rn = getArraySize( downtoSizeT(_activeBlock->getRemainSize()) );
      ...
      #ifdef MEM_DEBUG
         cout << "New MemBlock... " << _activeBlock ;
      #endif // MEM_DEBUG
      // 6. At the end, print out the acquired memory address
      if(!_activeBlock->getMem(t,ret))throw bad_alloc();
      #ifdef MEM_DEBUG
         cout << "Memory acquired... " << ret << endl << endl;
      #endif // MEM_DEBUG
      return ret;
   }
