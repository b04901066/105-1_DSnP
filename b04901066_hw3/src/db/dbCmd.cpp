...
DBAppendCmd::exec(const string& option)
{
   ...
   if(doRow){
      ...
      for(size_t i = 0; i < (dbtbl.nCols()) ; ++i ){
         if( i < (options.size()-1) ){
            int input_n;
            if(options[i+1] == "-" )input_nums.push_back(INT_MAX);
            else if(!myStr2Int( options[i+1] , input_n )) {
               cerr << "Error: " << options[i+1] << " is not a number!!\n";
               return CMD_EXEC_ERROR;
            }
            else input_nums.push_back(input_n);
         }
         else input_nums.push_back(INT_MAX);
      }
      dbtbl.addRow(input_nums);
   }
   else{
      vector<int> input_nums;
...
DBPrintCmd::exec(const string& option)
{
   ...
   int rowIdx = 0 , colIdx = 0 , tbl_r = dbtbl.nRows() , tbl_c = dbtbl.nCols();
   if(myStrNCmp("-Row", options[0], 2) == 0){
      if( options.size() > 2 ){
         return CmdExec::errorOption(CMD_OPT_EXTRA,options[2]);
      }
      if(!myStr2Int( options[1] , rowIdx ))
         return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[1]);
      if(!checkRowIdx(options[1], tbl_r))return CMD_EXEC_ERROR;
      ...
      if(!myStr2Int( options[1] , colIdx ))
         return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[1]);
      if(!checkColIdx(options[1], tbl_c))return CMD_EXEC_ERROR;
   ...
   else if(myStr2Int( options[0] , rowIdx )){
      if(!checkRowIdx(options[0], tbl_r))return CMD_EXEC_ERROR;
      if( options.size() > 2 ){
         return CmdExec::errorOption(CMD_OPT_EXTRA,options[2]);
      }
      if(!myStr2Int( options[1] , colIdx )){
      ...
      if(!checkColIdx(options[1], tbl_c))return CMD_EXEC_ERROR;
      dbtbl.printData(cout, dbtbl[rowIdx][colIdx], true);
      cout<<endl;
   }
   else{
      cerr << "Error: " << options[0] << " is not a number!!\n" << endl;
               return CMD_EXEC_ERROR;
   }
   return CMD_EXEC_DONE;
}
